...
COMMENT ON COLUMN BUILDING.STATUS IS 'Activity status';
...
CREATE UNIQUE INDEX BUILDING_I001 ON BUILDING (ID, NAME, NO, STREET, CITY, DEPARTMENT, COUNTRY);
CREATE INDEX BUILDING_I002 ON BUILDING (CITY, DEPARTMENT, COUNTRY);
CREATE INDEX BUILDING_I003 ON BUILDING (DEPARTMENT, COUNTRY);
CREATE INDEX BUILDING_I004 ON BUILDING (COUNTRY);
CREATE INDEX BUILDING_I005 ON BUILDING (STATUS, ID);

CREATE TABLE UNIT (
        BUILDING_ID VARCHAR(36) NOT NULL ,
        NAME VARCHAR(32) NOT NULL , 
        STATUS VARCHAR(1) NOT NULL ,
        BASE_SHARE NUMERIC(7,5) NOT NULL ,
        CRTU VARCHAR(64) NOT NULL ,
        CRTD TIMESTAMP NOT NULL ,
        UPDU VARCHAR(64) NOT NULL ,
        UPDD TIMESTAMP NOT NULL,
        PRIMARY KEY (BUILDING_ID, NAME),
        CONSTRAINT UNIT_C001 FOREIGN KEY (BUILDING_ID) REFERENCES BUILDING (ID));

COMMENT ON TABLE UNIT IS 'Units' ;

COMMENT ON COLUMN UNIT.BUILDING_ID IS 'Building ID';
COMMENT ON COLUMN UNIT.NAME IS 'Unit name';
COMMENT ON COLUMN UNIT.STATUS IS 'Activity status';
...
CREATE INDEX UNIT_I001 ON UNIT (STATUS, BUILDING_ID, NAME);
...
COMMENT ON COLUMN OWNER.STATUS IS 'Activity status';
...
CREATE INDEX OWNER_I001 ON OWNER (STATUS, ID);
...
COMMENT ON COLUMN CONTACT.STATUS IS 'Activity tatus';
COMMENT ON COLUMN CONTACT.FNAME IS 'First name';
COMMENT ON COLUMN CONTACT.LNAME IS 'Last name';
COMMENT ON COLUMN CONTACT.ADDRESS1 IS 'Address 1';
COMMENT ON COLUMN CONTACT.ADDRESS2 IS 'Address 2';
COMMENT ON COLUMN CONTACT.CITY IS 'City';
COMMENT ON COLUMN CONTACT.DEPARTMENT IS 'Prov/State/Department';
COMMENT ON COLUMN CONTACT.CITY IS 'City';
COMMENT ON COLUMN CONTACT.COUNTRY IS 'Country';
COMMENT ON COLUMN CONTACT.ZIP IS 'Postal code';
COMMENT ON COLUMN CONTACT.PHONE1 IS 'Phone 1';
COMMENT ON COLUMN CONTACT.PHONE2 IS 'Phone 2';
...
CREATE INDEX CONTACT_I001 ON CONTACT (LNAME, FNAME, COUNTRY, DEPARTMENT, CITY);
CREATE INDEX CONTACT_I002 ON CONTACT (COUNTRY, DEPARTMENT, CITY);
CREATE INDEX CONTACT_I003 ON CONTACT (ZIP);
CREATE INDEX CONTACT_I004 ON CONTACT (STATUS, ID);
...
CREATE TABLE EXERCICE (
        ...
        CONSTRAINT EXERCICE_C001 FOREIGN KEY (BUILDING_ID) REFERENCES BUILDING (ID));
...
CREATE TABLE BUILDING_OWNER (
        ...
        CONSTRAINT BUILDING_OWNER_C001 FOREIGN KEY (BUILDING_ID, UNIT_NAME) REFERENCES UNIT (BUILDING_ID, NAME),
        CONSTRAINT BUILDING_OWNER_C002 FOREIGN KEY (OWNER_ID) REFERENCES OWNER (ID));
...
COMMENT ON COLUMN BUILDING_OWNER.STATUS IS 'Activity status';
...
CREATE INDEX BUILDING_OWNER_I001 ON BUILDING_OWNER (STATUS, BUILDING_ID, UNIT_NAME, OWNER_ID);
...
CREATE TABLE CAPTION (
        ...
        UPDU VARCHAR(64) NOT NULL ,
        UPDD TIMESTAMP NOT NULL,
        PRIMARY KEY (ID, APP_ID, LANG_ID),
        CONSTRAINT CAPTION_C001 FOREIGN KEY (LANG_ID) REFERENCES LANGUAGE (ID),
        CONSTRAINT CAPTION_C002 FOREIGN KEY (APP_ID) REFERENCES APPLICATION (ID));
...
CREATE INDEX CAPTION_I001 ON CAPTION (APP_ID, LANG_ID, ID);
